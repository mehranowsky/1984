...
try:
    with open('worlists/js.json', 'r') as file:
...
pattern = re.compile(r'\.(' + '|'.join([
    ...
    'image', 'rf'
]) + r')$|jquery.*\.js$|gtm.*\.js$|analytics\.js$|gtag\.js$|googletagmanager\.js$|adsbygoogle\.js$|fbevents\.js$|widgets\.js$|iframe_api$|player\.js$|maps\.googleapis\.com$|intercom$|cdnjs\.com$|maxcdn\.com$|bootstrap.*\.js$')
        ...
        try:
            ...
            for endpoint in api_endpoints:
                try:
                    ...
                    raw_data = f"{api_response.headers}\n{api_response.content.decode('utf-8', errors='replace')}"
                    found_phrases = [phrase for phrase in js_phrases if phrase in raw_data]
                    if found_phrases:
